...
class Model:
  ...
  def __init__(self, game):
    ...
    self.weights = torch.zeros(self.param_count)
  ...
  def get_action(self, x, device='cpu'):
    ...
    annOut = ann.act(self.wVec, self.aVec, self.input_size, self.output_size, x, device)
    action = ann.selectAct(annOut, self.action_select, device)
...
def simulate(model, train_mode=False, render_mode=True, num_episode=5, seed=-1, max_len=-1, device='cpu'):
  ...
  for episode in range(num_episode):
    ...
    for t in range(max_episode_length):
      ...
      obs = torch.tensor(obs, device=device)  # Translate to torch.tensor
      model.wVec = torch.tensor(model.wVec, device=device, dtype=torch.float)
      action = model.get_action(obs)
      action = action.numpy()  # Revert torch.tensor to np.array
